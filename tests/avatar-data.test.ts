...
import { getAvatarDeliveryUrl, parseAvatarDataUrl, toExactAvatarArrayBuffer } from "@/lib/avatar-data";
...
describe("avatar delivery", () => {
  ...
  it("copies only the avatar bytes when the source uses a larger backing buffer", () => {
    const backingBuffer = Uint8Array.from([99, 99, 1, 2, 3, 99]).buffer;
    const avatarView = new Uint8Array(backingBuffer, 2, 3);

    const responseBuffer = toExactAvatarArrayBuffer(avatarView);

    expect(responseBuffer.byteLength).toBe(3);
    expect(Array.from(new Uint8Array(responseBuffer))).toEqual([1, 2, 3]);
  });
